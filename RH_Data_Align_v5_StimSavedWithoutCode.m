...
clc;
close all;
foldername = 'D:\Cornell Graduate Work\Humidity  ephys data\Ajinkya_ephys\Mar 27';
filename = 'stim_2'; % Stimulus File -  Don't put any .m or .mat here
hygrosensorfilename = 'rec_2'; % EPhys File - Don't put any .m or .mat here (Remember to convert .daq to .mat)
edited_ephys_filename = 'times_rec_2';

saveVarsMatfilename = strcat(foldername,'/', filename, '.mat');
saveVarsMat = load(saveVarsMatfilename);
StimCodeDatafilename = strcat(foldername,filename, '.m');
%StimCodeData= uiimport(StimCodeDatafilename);

%absStimTimeChar = StimCodeData.Male_RH_fast_sine_wave_stim_1{7}; % Change filename;
absStimTime = saveVarsMat.abstime;                             % Extracting Start Time
%absEndTimeChar = StimCodeData.Male_RH_fast_sine_wave_stim_1{11};
absEndTime = saveVarsMat.endtime;                               % Extracting End Time
%iChar = StimCodeData.Male_RH_fast_sine_wave_stim_1{13};
i = saveVarsMat.i;                                                  % Extracting time that was set for saving, typically 2 minutes.
...
xlim([Sel_start,Sel_end]);ylim([30,50]);
...
set(gcf,'renderer','painters') % important to get the figures xported in the right format
%set(gca,'Fontsize',15)
...
plot([cluster1_time,cluster1_time], [-3, 3], '-r');hold on;set(gcf,'renderer','painters');set(gca,'TickDir','out');

cluster3_time = edited_EphysTimeArray_sel(sel_cluster_class == 2);ylim([-5,5]);xlim([Sel_start,Sel_end]);
plot([cluster3_time,cluster3_time], [-1, 1], '-b');set(gcf,'renderer','painters');set(gca,'TickDir','out');
...
points1 = movmean(edges1,2); points1 = points1(2:end);
points3 = movmean(edges3,2); points3 = points3(2:end);

figure(2); hold on;
subplot(num_subplots,1,3);ylim([0,25]);xlim([Sel_start,Sel_end]);hold on;
plot(points1, cluster1_freq, 'r');set(gcf,'renderer','painters');set(gca,'TickDir','out');hold on;
plot(points3, cluster3_freq, 'b');set(gcf,'renderer','painters');set(gca,'TickDir','out');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Added this on 13th Jan 2022 for scatterplots
...
plot3(rh_array_points1_3d,rateofchange_points1_3d,cluster1_freq,'o','Color','r','MarkerSize',10,...
...
plot3(rh_array_points3_3d,rateofchange_points3_3d,cluster3_freq,'o','Color','b','MarkerSize',10,...
...
figure(5); hold on;
f1 = fit([rh_array_points1_3d', rateofchange_points1_3d'],cluster1_freq','poly11', 'Exclude', cluster1_freq == 0);
plot3(rh_array_points1_3d,rateofchange_points1_3d,cluster1_freq,'o','Color','r','MarkerSize',10,...
    'MarkerFaceColor','#ffb6c1');
view([-40 33]);
plot( f1, [rh_array_points1_3d',rateofchange_points1_3d'], cluster1_freq');
xlabel('Relative Humidity');
ylabel('Slope in RH');
zlabel('Impulse/s');

figure(6); hold on;
f3 = fit([rh_array_points3_3d', rateofchange_points3_3d'],cluster3_freq','poly11','Exclude',  cluster3_freq == 0);
plot3(rh_array_points3_3d,rateofchange_points3_3d,cluster3_freq,'o','Color','b','MarkerSize',10,...
    'MarkerFaceColor','#D9FFFF');
view([-40 33]);
plot( f3, [rh_array_points3_3d',rateofchange_points3_3d'], cluster3_freq');
xlabel('Relative Humidity');
ylabel('Slope in RH');
zlabel('Impulse/s');
